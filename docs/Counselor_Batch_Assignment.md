...
The responsibility boundary remains unchanged across AI systems: **AI makes academic decisions; deterministic code expresses, checks and publishes them.**
...
Process **one target end-to-end before moving to the next target assigned to the same AI production agent**.
...
A target is not considered processed merely because its comparator, alternative concepts, course sets or schedules have been worked out. Before beginning work on the next target, the current target must have:
...
Different contributors may work on different explicitly assigned targets in parallel, but GitHub production publication must remain serialized under Section 6. Do not use parallel preparation as a reason to push several new production decisions at once.
...
- do not substitute a later target for a blocked selected or assigned target;
...
Continue with the remaining assigned or selected targets and report the exception.
