...
Then construct the broadest coherent UCR programme that addresses that organising question from several relevant perspectives.

A course belongs in the thematic programme because it helps answer the organising question, not simply because it is broad, interesting or available.
...
7. only then use the enriched UCR course database to construct the three UCR programmes;
8. schedule and mechanically validate all three UCR programmes;
9. compare all four completed curricula;
10. assign the approved descriptive labels;
11. select a reusable explanatory-note type only if needed;
12. create the canonical counselor record, including the academic rationale;
13. add/update the deterministic comparison library;
14. run structural and academic quality control;
15. flag exceptions rather than inventing missing facts.
