  ...
  tags$head(
    ...
    tags$script(HTML("
      $(document).on('click', '.course-link', function() {
        ...
        if (code) {
          Shiny.setInputValue('course_click', String(code), {priority: 'event'});
        }
      });
    "))
  ),
...
server <- function(input, output, session) {
  ...
  course_lookup <- reactiveVal(NULL)
  ...
  observeEvent(input$unlock_pathway, {
    ...
    tryCatch({
      ...
      lookup <- fetch_json(paste0(PILOT_DATA_BASE, "/course_lookup.json"))
  ...
  observeEvent(input$course_click, {
    ...
    description <- lookup$descriptions[[code]]
